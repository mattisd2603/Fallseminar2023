...
It is included and further developed within the BIDScoin toolbox https://github.com/Donders-Institute/bidscoin 
...
def readCMRRPhysio(fn: Union[str,Path], showsamples: int=0) -> dict:
    """
    ...
    *_Info.log) generated by >=R013 sequences, or a single encoded "_PHYSIO" DICOM file generated by >=R015 sequences. It
    ...
    # If we don't have an encoded DICOM, check what text log files we have
        ...
        if not fnINFO.is_file():
            LOGGER.error(f"{fnINFO} not found"); raise FileNotFoundError  # noqa: E702
    ...
    if foundECG and ECG.any():
        if sum(ECG[:,0]): physio['ECG1'] = ECG[:,0]  # noqa: E701
        if sum(ECG[:,1]): physio['ECG2'] = ECG[:,1]  # noqa: E701
        if sum(ECG[:,2]): physio['ECG3'] = ECG[:,2]  # noqa: E701
        if sum(ECG[:,3]): physio['ECG4'] = ECG[:,3]  # noqa: E701
    ...
    print(slicemap[0:2,15,15,0]+firsttime)
    test_load = nib.load('Testdatensätze/20220809/run1/20220809074422_12_1.3.12.2.1107.5.2.43.67036.2022080907482564175503987.0.0.0_rfMRI_REST_AP.nii').get_fdata()
    test_load.shape
    ...
    def time_stamps_to_image_conformation(slicemap, mrt_data, firsttime):
        ...
        print(slicemap.shape)
        print(mrt_data[50,50,15,18])
        all_data = np.zeros((x,y,slice,volume,3))
        for k in range(slice):
            for l in range(volume):
                time_start = firsttime + slicemap[0,l, k, 0]
                time_end = firsttime + slicemap[1,l, k, 0]
                all_data[:,:,k,l,0] = mrt_data[:,:,k,l]
                all_data[:,:,k,l,1] = time_start
                all_data[:,:,k,l,2] = time_end
        return all_data

    all_data = time_stamps_to_image_conformation(slicemap, test, firsttime)
    print(all_data[50,50,15,18])

    Respiration_Raw = physio['RESP']
    Puls_Raw = physio['PULS']

    def delete_zeros(data_input):
        #Spline Interpolation probieren -> genauere Interpolation, linker negativpeak muss entfernt werden
        data = np.zeros(np.prod(data_input.shape))
        low = np.mean(data_input)
        high = np.mean(data_input)
        for i in range(np.prod(data.shape)):
            if data_input[i]<=100:
                if i == 0:
                    data[i] = low
                if i == np.prod(data.shape)-1:
                    data[i] = low
                else:
                    for under in range(i-1,0,-1):
                        if data_input[under]>100:
                            low = data_input[under]
                            break
                    for over in range(i+1,np.prod(data_input.shape),1):
                        if data_input[over]>100:
                            high = data_input[over]
                            break
                    data[i] = (low+high)/2
            else:
                data[i] = data_input[i]
    ...
    def lowpass(data_input, lowcut, fs, order=5):
        #Wir sollten einen Moving Average Filter probieren (Werte: 10,50,...)
        #Phasenversatz betrachten -> Samplerate mit einfließen lassen, im Spektralraum Filtern -> Funktion: FiltFilt,
        #Resampling der gefilterten Funktion, dabei Mittelwert abziehen
        nyq = 0.5 * fs
        low = lowcut / nyq
        b, a = signal.butter(order, low, btype='low')
        data = signal.lfilter(b, a, data_input)
        #Erst Lowpass, dann Downsampling, um Aliasing zu verhindern
        data = signal.resample(data_input, num=int(np.prod(data_input.shape)/329)) #?
        print(data.shape)
        return data

    fs = 500
    lowcut = 2 #lauft Besprechung möglicherweise 3 besser
    Puls_new = lowpass(delete_zeros(Puls_Raw), lowcut, fs)
    Respiration_new = lowpass(delete_zeros(Respiration_Raw), lowcut, fs)
    #data = np.zeros(np.prod(data_input.shape))
    spectrum = np.abs(np.fft.fft(Puls_new))
    freq = np.fft.fftfreq(Puls_new.size,(1/500))
    plt.figure('No_Zeros', figsize=(15,6))
    plt.subplot(1,4,1, title='Respiration')
    plt.plot(physio['RESP'])
    plt.subplot(1,4,2, title='Respiration verarbeitet')
    plt.plot(Respiration_new)
    plt.subplot(1,4,3, title='Puls')
    plt.plot(physio['PULS'])
    plt.subplot(1,4,4, title='Puls verarbeitet')
    #plt.plot(freq, spectrum)
    plt.plot(Puls_new)
    plt.show()

    #Und nipy.GLMFIT, um das Modell zu fitten -> anschauen
...
def main():
    ...
    readCMRRPhysio(fn=args.filename, showsamples=args.showsamples)
